...
void CMini_Map::Tick(_double TimeDelta)
{
	...
	if (m_UI_Desc.m_Type == 3) {
		...
		if (m_Map_Type == 0) {
			...
			// 1) Player 중심 UV 좌표를 기준으로 미니맵에 표시할 영역(window) 계산
			...
			// 2) UV 범위가 0~1 밖으로 벗어나지 않도록 clamp 처리
			if (m_UV_Cut_MinX <= 0.f) m_UV_Cut_MinX = 0.f;
			if (m_UV_Cut_MaxX >= 1.f) m_UV_Cut_MaxX = 1.f;

			if (m_UV_Cut_MinY <= 0.f) m_UV_Cut_MinY = 0.f;
			if (m_UV_Cut_MaxY >= 1.f) m_UV_Cut_MaxY = 1.f;
